...
export type PageAnimation = 'slide' | 'scroll' | 'none'
...
export interface ReaderSettings { enabled: boolean; openMode: 'newTab' | 'rightTab' | 'bottomTab' | 'newWindow'; tocPosition: TocPosition; pageAnimation: PageAnimation; columnMode: ColumnMode; theme: string; customTheme: ReadTheme; annotationMode: 'notebook' | 'document'; notebookId?: string; parentDoc?: DocInfo; linkFormat: string; textSettings: TextSettings; paragraphSettings: ParagraphSettings; layoutSettings: LayoutSettings; visualSettings: VisualSettings }

// ===== 预设主题 =====
export const PRESET_THEMES: Record<string, ReadTheme> = { default: { name: 'themeDefault', color: '#202124', bg: '#ffffff' }, almond: { name: 'themeAlmond', color: '#414441', bg: '#FAF9DE' }, autumn: { name: 'themeAutumn', color: '#414441', bg: '#FFF2E2' }, green: { name: 'themeGreen', color: '#414441', bg: '#E3EDCD' }, blue: { name: 'themeBlue', color: '#414441', bg: '#DCE2F1' }, night: { name: 'themeNight', color: '#fff6e6', bg: '#415062' }, dark: { name: 'themeDark', color: '#d5cecd', bg: '#414441' }, gold: { name: 'themeGold', color: '#b58931', bg: '#081010' } }
...
export const formatBookLink = (bookUrl: string, bookTitle: string, author: string, chapter: string, cfi: string, text: string, format: string) => applyTemplate(format, { '书名|{{title}}': bookTitle, '作者|{{author}}': author, '章节|{{chapter}}': chapter, '位置|{{location}}': cfi, '链接|{{url}}': `sireader://open?url=${bookUrl}&cfi=${cfi}`, '文本|{{text}}': text })
export const parseBookLink = (url: string): { bookUrl: string; cfi: string } | null => { try { const m = url.match(/^sireader:\/\/open\?(.+)$/); if (!m) return null; const p = new URLSearchParams(m[1].replace(/&amp;/g, '&')); const u = p.get('url'), c = p.get('cfi'); return u && c ? { bookUrl: u, cfi: c } : null } catch { return null } }
