...
export const formatBookLink = (bookUrl: string, bookTitle: string, author: string, chapter: string, cfi: string, text: string, format: string, note = '', image = '', id = '') => applyTemplate(format, { '书名|{{title}}': bookTitle, '作者|{{author}}': author, '章节|{{chapter}}': chapter, '位置|{{location}}': cfi, '链接|{{url}}': `sireader://open?url=${id?`${bookUrl}_${id}`:bookUrl}&cfi=${cfi}`, '文本|{{text}}': text, '笔记|{{note}}': note, '截图|{{image}}': image }).replace(/> \n/g, '').replace(/\n\n+/g, '\n')
