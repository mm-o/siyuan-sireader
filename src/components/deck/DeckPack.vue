<template>
  <div class="deck-pack">
    ...
    <template v-for="pack in displayTree" :key="pack.id">
      <!-- 父卡组分组头 -->
      <div v-if="pack.isParent" class="sr-card sr-group" @click.stop="toggleExpand(pack.id)" :style="indent(pack)">
        <span class="sr-bar" :class="{collapsed:!expanded.has(pack.id)}" :style="{background:pack.color}"></span>
      ...
      <!-- 普通卡组 -->
      <div v-else class="sr-card" :class="{'sr-card-edit':isEditing(pack)}" :style="indent(pack)">
        ...
        <div v-if="isEditing(pack)" class="pack-editor">
          ...
          <div class="property-table">
            <template v-for="prop in propConfig" :key="prop.key">
              <div v-if="!prop.condition || prop.condition()" class="prop-row" :class="{expanded:expandedProp===prop.key}" @click.stop="toggleProp(prop.key)">
        ...
        <div v-else class="sr-main">
          ...
          <Transition name="fade">
            <div v-if="removing===pack.id" class="sr-confirm b3-chip b3-chip--middle" @click.stop>
              ...
              <button @click="confirmDelete(pack.id)" class="b3-button b3-button--text btn-delete">删除</button>
            </div>
          </Transition>
...
const expandedProp = ref<string | null>(null)
const tagInput = ref('')
...
const emojiLoadCount = ref(1)

// 工具函数
const indent = (pack: any) => ({ marginLeft: `${pack.level * 20}px` })
const isEditing = (pack: any) => editing.value === pack.id
const currentData = (pack: any) => isEditing(pack) ? form.value : pack
const select = (key: string, val: any) => { form.value[key] = val; expandedProp.value = null }
const resetEditor = () => { showPreview.value = false; expandedProp.value = null; templateInfo.value = null }
const arrayOps = (key: 'tags' | 'fields', input: typeof tagInput) => ({
  toggle: (val: string) => { const idx = form.value[key].indexOf(val); idx > -1 ? form.value[key].splice(idx, 1) : form.value[key].push(val) },
  remove: (val: string) => form.value[key].splice(form.value[key].indexOf(val), 1),
  add: () => { const val = input.value.trim(); if (val && !form.value[key].includes(val)) { form.value[key].push(val); input.value = '' } }
})
...
const propConfig = computed(() => [
  {
    key: 'icon',
    label: '图标',
    preview: () => h('span', { class: 'prop-preview' }, form.value.icon || '未设置'),
    content: () => props.loadingEmojis ? h('div', { class: 'loading-hint' }, '加载中...') : h('div', { 
      class: 'emoji-container',
      onScroll: (e: Event) => {
        const el = e.target as HTMLElement
        if (el.scrollHeight - el.scrollTop - el.clientHeight < 100 && hasMoreEmojis.value) emojiLoadCount.value++
      }
    }, [
      h('div', { class: 'emoji-list' }, visibleEmojis.value.map(e => h('span', { class: 'emoji-item', onClick: () => select('icon', e) }, e))),
      hasMoreEmojis.value ? h('div', { class: 'loading-hint' }, '滚动加载更多...') : null
    ])
  },
  {
    key: 'color',
    label: '颜色',
    preview: () => h('span', { class: 'prop-preview' }, [h('span', { class: 'color-dot', style: { background: form.value.color } })]),
    content: () => h('div', { class: 'color-grid' }, COLORS.map(c => h('button', { class: ['color-btn', { active: form.value.color === c.v }], style: { background: c.v }, title: c.n, onClick: () => select('color', c.v) })))
  },
  {
    key: 'img',
    label: '题头图',
    preview: () => h('span', { class: 'prop-preview' }, form.value.titleImg ? '已设置' : '未设置'),
    content: () => !imgs.value.length ? h('div', { class: 'loading-hint' }, '暂无题头图') : h('div', { class: 'img-grid' }, [
      ...imgs.value.map(img => h('div', { class: 'img-item', style: img.titleImg, onClick: () => select('titleImg', img.titleImg) })),
      form.value.titleImg ? h('div', { class: 'img-item clear', onClick: () => select('titleImg', '') }, '清除') : null
    ].filter(Boolean))
  },
  {
    key: 'tag',
    label: '标签',
    preview: () => h('span', { class: 'prop-preview' }, form.value.tags.length ? `${form.value.tags.length} 个` : '未设置'),
    content: () => h('div', {}, [
      h('input', { class: 'prop-input', placeholder: '搜索或添加...', value: tagInput.value, onInput: (e: any) => tagInput.value = e.target.value, onKeyup: (e: KeyboardEvent) => e.key === 'Enter' && tagOps.value.add(), onClick: (e: Event) => e.stopPropagation() }),
      form.value.tags.length ? h('div', { class: 'tag-chips' }, form.value.tags.map(tag => h('span', { class: 'tag-chip', onClick: () => tagOps.value.remove(tag) }, `${tag} ×`))) : null,
      filteredTags.value.length ? h('div', { class: 'tag-options' }, filteredTags.value.map(tag => h('div', { class: 'tag-opt', onClick: () => tagOps.value.toggle(tag) }, [h('span', {}, tag), form.value.tags.includes(tag) ? h('span', { class: 'check' }, '✓') : null]))) : null,
      tagInput.value && !props.allTags.includes(tagInput.value) ? h('div', { class: 'tag-opt add', onClick: tagOps.value.add }, `+ "${tagInput.value}"`) : null
    ].filter(Boolean))
  },
  {
    key: 'desc',
    label: '描述',
    preview: () => h('span', { class: 'prop-preview' }, form.value.desc ? '已设置' : '未设置'),
    content: () => h('textarea', { class: 'prop-textarea', placeholder: '描述（可选）', value: form.value.desc, onInput: (e: any) => form.value.desc = e.target.value })
  },
  {
    key: 'fields',
    label: '字段',
    preview: () => h('span', { class: 'prop-preview' }, form.value.fields.length ? `${form.value.fields.length} 个` : '未设置'),
    content: () => h('div', {}, [
      h('input', { class: 'prop-input', placeholder: '搜索或添加字段...', value: fieldInput.value, onInput: (e: any) => fieldInput.value = e.target.value, onKeyup: (e: KeyboardEvent) => e.key === 'Enter' && fieldOps.value.add(), onClick: (e: Event) => e.stopPropagation() }),
      form.value.fields.length ? h('div', { class: 'tag-chips' }, form.value.fields.map(field => h('span', { class: 'tag-chip', onClick: () => fieldOps.value.remove(field) }, `${field} ×`))) : null,
      fieldInput.value ? h('div', { class: 'tag-opt add', onClick: fieldOps.value.add }, `+ "${fieldInput.value}"`) : null
    ].filter(Boolean))
  },
  {
    key: 'qfmt',
    label: '前面模板',
    condition: () => form.value.collectionId && form.value.ankiDeckId,
    content: () => loadingTemplate.value ? h('div', { class: 'loading-hint' }, '加载中...') : h('textarea', { class: 'prop-textarea code', placeholder: '前面模板', value: form.value.qfmt, onInput: (e: any) => form.value.qfmt = e.target.value })
  },
  {
    key: 'afmt',
    label: '背面模板',
    condition: () => form.value.collectionId && form.value.ankiDeckId,
    content: () => loadingTemplate.value ? h('div', { class: 'loading-hint' }, '加载中...') : h('textarea', { class: 'prop-textarea code', placeholder: '背面模板', value: form.value.afmt, onInput: (e: any) => form.value.afmt = e.target.value })
  },
  {
    key: 'css',
    label: '样式 CSS',
    content: () => h('textarea', { class: 'prop-textarea code', placeholder: '自定义卡片样式（可选）\n例如：.card { font-size: 20px; }', value: form.value.modelCss, onInput: (e: any) => form.value.modelCss = e.target.value })
  }
])
...
const displayTree = computed(() => {
  ...
  const flatten = (items, lv = 0) => items.forEach(item => {
    ...
    const stat = hasChild ? count(item) : { total: 0, enabled: 0 }
...
const loadTemplateInfo = async () => {
  ...
  try {
    ...
    const ankiDb = await getAnkiDb(form.value.collectionId)
    if (!ankiDb) return
...
const renderPreview = () => {
  ...
  form.value.fields.forEach((field: string, idx: number) => {
    sampleData[field] = idx === 0 ? '<div style="font-size:24px;font-weight:600;margin:20px 0">示例问题</div>' 
      : idx === 1 ? '<div style="font-size:18px;margin:20px 0">这是答案的示例内容</div>' 
      : `示例${field}`
  })
  const parse = (html: string): string => {
    return html
      .replace(/\{\{#(\w+)\}\}([\s\S]*?)\{\{\/\1\}\}/g, (_, f, c) => sampleData[f] ? parse(c) : '')
      .replace(/\{\{\^(\w+)\}\}([\s\S]*?)\{\{\/\1\}\}/g, (_, f, c) => !sampleData[f] ? parse(c) : '')
      .replace(/\{\{(\w+):(\w+)\}\}/g, (_, filter, field) => sampleData[field] || '')
      .replace(/\{\{(\w+)\}\}/g, (m, f) => f === 'FrontSide' ? m : (sampleData[f] || ''))
  }
...
const edit = (pack: Pack) => { 
  form.value = { name: pack.name || '', desc: pack.desc || '', icon: pack.icon || '', color: pack.color || '#667eea', titleImg: pack.titleImg || '', tags: pack.tags || [], fields: (pack as any).fields || [], parent: pack.parent, modelCss: pack.modelCss || '', collectionId: pack.collectionId || '', ankiDeckId: pack.ankiDeckId || 0, qfmt: (pack as any).qfmt || '', afmt: (pack as any).afmt || '' }
