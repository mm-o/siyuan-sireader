...
const ANKI_PATH = '/data/storage/petal/siyuan-sireader/anki'
...
export const deletePack = async (id: string) => {
  ...
  if (pack.collectionId && !(await db.getDecks()).some((d: any) => d.collectionId === pack.collectionId)) {
...
export const updatePackStats = async (id: string) => { await (await getDatabase()).updateDeckStats(id); notify(); return true }
...
export const importApkg = async (file: File) => {
  ...
  try {
    ...
    if ((await db.getCollections()).find((c: any) => c.name === name)) return showMessage('该卡组已存在', 3000, 'info', msgId as any)
    ...
    showMessage(`✓ 已导入 ${created} 个卡组`, 2000, 'info', msgId as any)
    notify()
    clearAnkiDbCache()
    return result
  } catch (e) {
