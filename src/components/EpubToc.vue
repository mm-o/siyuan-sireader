<template>
  <div class="epub-toc fn__flex-column" style="height:100%">
    <div v-if="searchable" class="toc-search">
      <input v-model="searchQuery" class="b3-text-field fn__block" :placeholder="i18n?.searchChapter || '搜索章节...'" @input="handleSearch" />
      <svg class="search-icon"><use xlink:href="#iconSearch"></use></svg>
    </div>
    <div ref="tocBd" class="toc-body fn__flex-1">
      <div v-if="loading" class="toc-state ft__center"><div class="fn__loading"><div class="fn__loading-icon"></div></div><span>{{ i18n?.loadingToc || '加载目录中...' }}</span></div>
      <div v-else-if="markMode && !displayMarks.length" class="toc-state ft__center">{{ i18n?.noMarks || '暂无标记' }}</div>
      <div v-else-if="bookmarkMode && !displayBookmarks.length" class="toc-state ft__center">{{ i18n?.noBookmarks || '暂无书签' }}</div>
      <div v-else-if="!displayToc.length && !markMode && !bookmarkMode" class="toc-state ft__center">{{ searchQuery ? (i18n?.noMatchChapter || '未找到匹配的章节') : (i18n?.noToc || '暂无目录') }}</div>
      <div v-else-if="markMode" class="mark-list"><div class="toc-list"><TocItem v-for="(mark, i) in displayMarks" :key="i" :item="{ href: mark.cfi, label: mark.text }" :href="currentHref" :bookmarkMode="true" :markColor="mark.color" :lv="0" :i18n="i18n" @nav="handleMarkClick" @mark="() => emit('removeMark', mark.cfi)" /></div></div>
      <div v-else-if="bookmarkMode" class="toc-list"><TocItem v-for="(bm, i) in displayBookmarks" :key="i" :item="{ href: bm.href, label: bm.label }" :href="currentHref" :bookmarkMode="true" :lv="0" :i18n="i18n" @nav="handleBookmarkClick" @mark="handleRemoveBookmark" /></div>
      <div v-else class="toc-list"><TocItem v-for="(item, i) in displayToc" :key="i" :item="item" :href="currentHref" :prog="progress" :bookmarked="isBookmarked" :lv="0" :i18n="i18n" @nav="handleNavigate" @mark="handleToggleBookmark" /></div>
    </div>
  </div>
</template>
...
interface Props { toc: TocItemData[]; currentHref?: string; loading?: boolean; searchable?: boolean; progress?: Record<string, number>; bookmarks?: Bookmark[]; marks?: MarkData[]; baseUrl?: string; i18n? }
...
const TocItem = {
  props: { item: Object, href: String, prog: Object, bookmarked: Function, bookmarkMode: Boolean, markColor: String, lv: Number, i18n },
  emits: ['nav', 'mark'],
  setup(p: any, { emit }: any) {
    ...
    return () => h('div', {}, [
      h('div', { class: ['b3-list-item', { 'b3-list-item--focus': active.value }], style: itemStyle, onClick: () => emit('nav', p.item.href) }, [
        ...
        !p.bookmarkMode && !p.markColor && h('button', { class: 'b3-tooltips b3-tooltips__w', 'aria-label': marked.value ? (p.i18n?.removeBookmark || '移除书签') : (p.i18n?.addBookmark || '添加书签'), style: `${BTN};opacity:${marked.value ? 1 : 0};transition:opacity .2s`, onClick: (e: Event) => (e.stopPropagation(), emit('mark', p.item.href, p.item.label)) }, h('svg', { style: `width:14px;height:14px;color:var(--b3-theme-${marked.value ? 'error' : 'on-surface'})` }, h('use', { 'xlink:href': '#iconBookmark' }))),
        (p.bookmarkMode || p.markColor) && h('button', { class: 'b3-tooltips b3-tooltips__w', 'aria-label': p.markColor ? (p.i18n?.deleteMark || '删除标记') : (p.i18n?.deleteBookmark || '删除书签'), style: `${BTN};opacity:0;transition:opacity .2s${p.markColor ? ';position:absolute;top:8px;right:8px' : ''}`, onClick: (e: Event) => (e.stopPropagation(), emit('mark', p.item.href, p.item.label)) }, h('svg', { style: 'width:14px;height:14px;color:var(--b3-theme-error)' }, h('use', { 'xlink:href': '#iconTrashcan' })))
      ]),
      progress.value > 0 && h('div', { class: 'toc-progress' }, h('div', { class: 'toc-progress-bar', style: `width:${progress.value}%` })),
      exp.value && hasSub.value && h('div', {}, p.item.subitems.map((sub: any, i: number) => h(TocItem, { key: i, item: sub, href: p.href, prog: p.prog, bookmarked: p.bookmarked, lv: p.lv + 1, i18n: p.i18n, onNav: (h: string) => emit('nav', h), onMark: (h: string, l: string) => emit('mark', h, l) })))
    ])
  }
}
</script>
