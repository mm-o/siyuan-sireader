...
export class PDFViewer {
  ...
  async getThumbnail(n: number, s = .2) { try { let p = this.pages.get(n); if (!p && this.pdf) { p = markRaw(await this.pdf.getPage(n)); this.pages.set(n, p) }; if (!p) return ''; const vp = p.getViewport({ scale: s }), c = document.createElement('canvas'), ctx = c.getContext('2d'); if (!ctx) return ''; c.width = vp.width; c.height = vp.height; await p.render({ canvasContext: ctx, viewport: vp, canvas: c }).promise; return c.toDataURL('image/jpeg', .8) } catch { return '' } }
