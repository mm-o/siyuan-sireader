...
- 修复 PDF 标注菜单因命令未注册导致 `sireader:copy-annotation-link` 报错和界面卡死的问题。命令现在会先注册并校验，再合并菜单；执行异常会被隔离，过期菜单引用会被清理。
- 修复 PDF 文件名包含括号、空格、网址、`+`、`#` 等特殊字符时，标注回链无法跳转、无法悬浮预览并提示“书籍不存在”的问题，并兼容旧格式回链。
- 修复 EPUB 分页模式下鼠标拖选到页面边缘无法自动翻页的问题，现在无需按住 Shift 即可连续跨页选择文本。

### 新增

- 新增 PDF 全功能思源原生快捷键，可在“设置 → 快捷键”中配置；命令名称统一使用 `PDF` 前缀，如“PDF 放大”“PDF 缩小”，与 EPUB 快捷键区分。
