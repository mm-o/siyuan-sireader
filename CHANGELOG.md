...
## 🏗️ [规划中] v0.9.0 - 架构重构 (2026-02-09 ~ 2026-02-18)

### 📋 重构目标

**统一数据库，精简架构，14 个文件，10 天完成，性能提升 2 倍！**

#### 核心目标
1. **统一数据层**: JSON → IndexedDB，所有数据统一存储
2. **简化架构**: ~20 文件 → 14 个核心文件
3. **消除重复**: 合并 PDF/EPUB 的重复逻辑
4. **提升性能**: 数据库查询 < 50ms
5. **易于维护**: 新增功能只需修改 1-2 个文件
...
- 📋 [需求文档](.kiro/specs/annotation-system-refactor/requirements.md)
- 🏗️ [设计文档](.kiro/specs/annotation-system-refactor/design.md)
- 📝 [任务清单](.kiro/specs/annotation-system-refactor/tasks.md)
- 📅 [开发计划](.kiro/specs/annotation-system-refactor/DEVELOPMENT_PLAN.md)
- 📖 [快速参考](.kiro/specs/annotation-system-refactor/QUICK_REFERENCE.md)
- 📚 [项目导航](.kiro/specs/annotation-system-refactor/README.md)
...
**统一阅读数据库**
- 创建 `reader-database.ts` 统一管理所有类型的标注
- 使用 SQLite 数据库替代 JSON 文件存储
- 支持的标注类型：高亮、笔记、形状、墨迹、书签
- 数据库路径：`/data/storage/petal/siyuan-sireader/reader-data.db`

**自动迁移功能**
- 首次使用时自动从 JSON 迁移到数据库
- 用户无感知，平滑过渡
- 保留原始 JSON 数据作为备份
- ✅ 已验证：EPUB 和 PDF 迁移成功

**降级方案**
- 数据库操作失败时自动降级到 JSON 存储
- 确保向后兼容，不影响现有功能

### 🔧 优化改进

**代码重构**
- 修改 `MarkManager` 使用数据库存储（~40行改动）
- 修改 `ShapeToolManager` 使用数据库存储（~30行改动）
- 修改 `InkToolManager` 使用数据库存储（~30行改动）
- 统一数据存储接口，消除代码重复
...
**墨迹标注渲染问题**
- 修复墨迹标注迁移时丢失 `rect` 字段的问题
- 修复墨迹预览渲染错误（`rect is not iterable`）
- 确保墨迹标注的边界框正确保存和加载

**Deck 导入问题**
- 临时禁用不存在的 `getCard()` 和 `getCardsSync()` 调用
- 避免阻塞核心功能测试
- 待后续修复词汇卡片功能
