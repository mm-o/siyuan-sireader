...
- 重构 PDF 和 EPUB 标注持久化逻辑，改为按标注 ID 顺序执行新增、更新和删除，避免反复创建、删除和刷新时，延迟的全量快照恢复已删除标注或覆盖较新的笔记。
- 修复自动同步思源文档后使用旧 PDF 标注对象回写块 ID 的问题；现在会合并到最新标注数据，避免覆盖用户刚完成的修改。
- 修复 PDF 删除标注后左侧列表暂时残留的问题；标注列表会在阅读器事件稳定后重新读取。
- 修复标注首次保存失败后立即删除，后续失败操作重放导致标注重新出现的问题；删除操作现在会作为同一记录的有序操作可靠执行。

### 优化

- 新增统一事务化存储层，结构化数据写入支持版本号、校验和、同文件顺序队列、跨窗口锁、写后重新读取校验、缓存失效和退出前任务排空。
- 设置、许可状态、阅读统计、页面脚本设置、书架索引、阅读进度和标注记录统一使用事务化读写，减少并发覆盖和异常退出导致的数据不一致。
- 写入失败时自动重试，并在同一记录的后续操作之前优先重放失败操作；PDF/EPUB 自动插入思源文档只在对应标注可靠写入并校验成功后触发。
- 跨文件修改使用 `transactions/` 下的可恢复预写日志；插件启动时会先恢复未完成事务，提交完成后自动清理日志。
- 托管的书籍、封面、背景、字体和词典改为临时写入、文件大小校验和原子发布，避免写入中断留下半成品文件。
- 首次从旧版 JSON 存储升级时，会先将原始数据一次性备份到 `backups/storage-v1/<时间戳>/`，验证备份后再迁移。该目录是静态迁移快照，不会持续更新，也不是当前数据源。

## v2.3.1（2026.9.23）

### 修复

- 修复正式版小程序码接口直接返回图片时，因前端重复请求和转码导致二维码生成失败或加载缓慢的问题；现在直接交由浏览器加载图片地址。
...
- PDF 顶部工具栏新增批注锁定开关：关闭时完成一次批注自动回到手形拖动模式，开启后持续保持当前批注工具。

### 优化

- 优化书架紧凑视图布局，统一分组与书籍行高，对齐思源文档树的层级标识，并规范状态与进度排列。
- PDF 翻页方式接入阅读设置，使用 EmbedPDF 官方 `instant` 与 `smooth` API，统一目录、标注及相邻翻页行为；同时清理重复的按钮拦截和自定义滚动逻辑，提升翻页稳定性。
...
- 修复部分 EPUB 跨章节脚注点击无响应或误显示整章注释的问题，兼容普通 `type="noteref"` / `type="rearnote"` 结构，并根据当前脚注内容自适应弹窗高度。
...
- 优化微信读书顶部按钮隐藏逻辑，关闭后彻底移除按钮，重新启用时按设置恢复，避免按钮残留或重复注册。
...
- 修复 PDF 标注菜单因命令未注册导致 `sireader:copy-annotation-link` 报错和界面卡死的问题。
- 修复 PDF 文件名包含特殊字符时标注回链无法跳转、无法预览并提示“书籍不存在”的问题。
- 修复 EPUB 分页模式下鼠标拖选到页面边缘无法自动翻页的问题，现在无需按住 Shift 即可连续跨页选择文本。

### 新增

- 新增 PDF 全功能思源原生快捷键，可在“设置 → 快捷键”中配置；命令统一使用 `PDF` 前缀。
...
- 暂不增加阅读字数统计：当前 EPUB/PDF 引擎没有低成本的原始字数 API，完整统计需要扫描全文或逐页提取文本，可能影响打开和阅读性能。
...
- PDF 标注改为通过 EmbedPDF 原生标注 API 保存和恢复，减少中间格式转换导致的错位与丢失风险。
